...
export default class DuaScreenPreferences extends ExtensionPreferences {
    ...
    _detect(showMsg = true) {
        try {
            ...
            for (const m of this._monitors) {
                ...
                m.wp = prev && prev.wp ? prev.wp : { x: 0, y: 0, z: 1 };
            }
    ...
    _build() {
        ...
        this._picBtns = [];
        const addPicBtn = (label, icon, cb) => {
            const b = this._btn(label, icon, cb);
            this._picBar.append(b);
            this._picBtns.push(b);
            return b;
        };
        addPicBtn('◀', null, () => this._nudgePic(-0.06, 0));
        addPicBtn('▶', null, () => this._nudgePic(0.06, 0));
        addPicBtn('▲', null, () => this._nudgePic(0, -0.06));
        addPicBtn('▼', null, () => this._nudgePic(0, 0.06));
        addPicBtn('−', null, () => this._zoomPic(1.12));
        addPicBtn('+', null, () => this._zoomPic(1 / 1.12));
    ...
    _updateWpButtons() {
        ...
        this._picLbl.label = hasSel
            ? _('Picture on ') + this._monitors[this._sel].name + ':'
    ...
    _updateInfo() {
        ...
        if (this._sel < 0 || this._sel >= this._monitors.length) {
            this._info.label = _('Click a display to select it. Right-drag moves its picture, Ctrl+scroll zooms it.');
            return;
        }
